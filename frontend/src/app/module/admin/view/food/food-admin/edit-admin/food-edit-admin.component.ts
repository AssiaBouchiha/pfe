...
@Component({
  selector: 'app-food-edit-admin',
  templateUrl: './food-edit-admin.component.html'
})
export class FoodEditAdminComponent extends AbstractEditController<FoodDto, FoodCriteria, FoodService>   implements OnInit {
   ...
   public async openCreateBrand(brand: string) {
        const isPermistted = await this.roleService.isPermitted('Brand', 'edit');
        if(isPermistted) {
             this.brand = new BrandDto();
