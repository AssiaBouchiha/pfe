...
@Component({
  selector: 'app-specimen-create-admin',
  templateUrl: './specimen-create-admin.component.html'
})
export class SpecimenCreateAdminComponent extends AbstractCreateController<SpecimenDto, SpecimenCriteria, SpecimenService>  implements OnInit {
   ...
   private _validSpecimenCode = true;
    ...
    const isPermistted = await this.roleService.isPermitted('Reason', 'add');
    if(isPermistted) {
         this.reason = new ReasonDto();
