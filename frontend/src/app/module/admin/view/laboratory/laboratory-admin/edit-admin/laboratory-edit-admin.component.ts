...
export class LaboratoryEditAdminComponent extends AbstractEditController<LaboratoryDto, LaboratoryCriteria, LaboratoryService>   implements OnInit {
   ...
   public addAntibiotic() {
       ...
       if (this.errorMessages.length === 0) {
            if(this.antibioticElement.id == null){
                this.item.antibiotic.push(this.antibioticElement);
            }else{
                const index = this.item.antibiotic.findIndex(e => e.id == this.antibioticElement.id);
                this.item.antibiotic[index] = this.antibioticElement;
            }
   ...
   public addNotifications() {
    ...
    private validateAntibioticCode(){
